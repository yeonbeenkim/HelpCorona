  ...
  void getData(int cases, String url) async {
   ...
   DataManager.jejuHospitalData = new List<dynamic>();
        for (int i = 0; i < data.length; i++) {
          ...
          // }
          switch (data[i][1]) {
            case '서울': DataManager.seoulHospitalData.add(data[i]); break;
            case '부산': DataManager.busanHospitalData.add(data[i]); break;
            case '대구': DataManager.daeguHospitalData.add(data[i]); break;
            case '인천': DataManager.incheonHospitalData.add(data[i]); break;
            case '광주': DataManager.gwangjuHospitalData.add(data[i]); break;
            case '대전': DataManager.daejeonHospitalData.add(data[i]); break;
            case '울산': DataManager.ulsanHospitalData.add(data[i]); break;
            case '세종': DataManager.sejongHospitalData.add(data[i]); break;
            case '경기': DataManager.gyeonggiHospitalData.add(data[i]); break;
            case '강원': DataManager.gangwonHospitalData.add(data[i]); break;
            case '충북': DataManager.chungbukHospitalData.add(data[i]); break;
            case '충남': DataManager.chungnamHospitalData.add(data[i]); break;
            case '전북': DataManager.jeonbukHospitalData.add(data[i]); break;
            case '전남': DataManager.jeonnamHospitalData.add(data[i]); break;
            case '경북': DataManager.kyeongbukHospitalData.add(data[i]); break;
            case '경남': DataManager.kyeongnamHospitalData.add(data[i]); break;
            case '제주': DataManager.jejuHospitalData.add(data[i]); break;
          }
        }
